...
/**
 *   	\file       incident_list.php
 *		\ingroup    incident
 *		\brief      List page for incident
 */
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('incident', 'incident', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("incident")) {
	accessforbidden('Module incident not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Incident';
	$objectlabel = 'Incident';
	$uploaddir = $conf->incident->dir_output;
...
//$help_url = "EN:Module_Incident|FR:Module_Incident_FR|ES:Módulo_Incident";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/incident/incident_card.php', 1).'?id='.((int) $id));
	exit;
}
...
// Project
	if (isModEnabled('project')) {
		...
		if ($permissiontoadd) {
			...
			$morehtmlref .= $form->form_project($_SERVER['PHP_SELF'].'?id='.$originObject->id, $originObject->socid, $originObject->fk_project, ($action == 'classify' ? 'projectid' : 'none'), 0, 0, 0, 1, '', 'maxwidth300');
		} else {
			if (!empty($originObject->fk_project)) {
				$proj = new Project($db);
				$proj->fetch($originObject->fk_project);
	...
	$morehtmlref .= '</div>';
	if (get_class($originObject) == Agsession::class){
		dol_agefodd_banner_tab($originObject, 'originId', '', 1, 'rowid', 'ref', $morehtmlref, '&type='.$type);
	}else{
...
if (!empty($originId) && !empty($type)) $param .= '&originId='.$originId.'&type='.$type;
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		$param .= '&originId='.$originId.'&type='.$type;
